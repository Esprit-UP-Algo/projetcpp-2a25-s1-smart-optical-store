...
bool Vente::ajouterProduitVente(int idVente, const QString &refProduit, int quantite, double prixUnitaire)
{
    productAddError.clear();  // Clear previous error
    
    if (idVente <= 0) {
        productAddError = QString("ID de vente invalide: %1").arg(idVente);
        qDebug() << "❌ Erreur:" << productAddError;
        return false;
    }
    
    if (refProduit.isEmpty()) {
        productAddError = "Référence produit vide";
        qDebug() << "❌ Erreur:" << productAddError;
        return false;
    }
    
    if (quantite <= 0) {
        productAddError = QString("Quantité invalide: %1").arg(quantite);
        qDebug() << "❌ Erreur:" << productAddError;
        return false;
    }
    ...
    if (!saleCheck.next()) {
        productAddError = QString("La vente #%1 n'existe pas dans la base de données").arg(idVente);
        qDebug() << "❌ Erreur:" << productAddError;
        return false;
    }
    ...
    if (!productCheck.exec()) {
        qDebug() << "Erreur lors de la vérification du produit (PRODUIT):" << productCheck.lastError().text();
        qDebug() << "Code erreur:" << productCheck.lastError().nativeErrorCode();
        qDebug() << "Tentative avec produit en minuscules...";
        productCheck.clear();
        ...
        if (!productCheck.exec()) {
            ...
            qDebug() << "Erreur (produit):" << productCheck.lastError().text();
            qDebug() << "Code erreur:" << productCheck.lastError().nativeErrorCode();
            return false;
        }
    }
    ...
    if (!productCheck.next()) {
        productAddError = QString("Le produit avec référence '%1' n'existe pas dans la base de données").arg(refProduit);
        qDebug() << "❌ Erreur:" << productAddError;
        qDebug() << "Vérifiez que la référence est correcte et que le produit existe bien";
        return false;
    }
    
    qDebug() << "✅ Produit vérifié - Référence:" << refProduit;
