...
Gclient1* Gclient1::getInstance(const QString &role, QWidget *parent)
{
    if (!instance || !QApplication::topLevelWidgets().contains(instance)) {
        instance = new Gclient1(role, parent);
...
Gclient1::Gclient1(const QString &role, QWidget *parent)
    : QMainWindow(parent),
    ...
    currentClientId(-1)
{
    ...
    // 🔹 إعداد اللوغو ليكون فوق كل العناصر (حتى العنوان) et cliquable
    ...
    if (ui->b1) { // Stock button
        if (role != "admin" && role != "stock") {
            ui->b1->setEnabled(false);
        }
    }
    
    if (ui->b2) { // Ventes button
        if (role != "admin" && role != "vente") {
            ui->b2->setEnabled(false);
        }
    }
    
    if (ui->b3) { // Employes button
        if (role != "admin" && role != "employe") {
            ui->b3->setEnabled(false);
        }
    }
    
    if (ui->b4) { // Clients button (self)
        // Always enabled for clients module
    }
    
    if (ui->b5) { // Fournisseur button
...
{
    // Stock - only accessible by admin or stock roles
...
{
    // Ventes - only accessible by admin or vente roles
...
{
    // Employes - only accessible by admin or employe roles
...
{
    // Fournisseur - only accessible by admin or fournisseur roles
